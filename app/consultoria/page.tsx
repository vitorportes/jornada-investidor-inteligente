...
const pageData = {
  ...
  processSteps: [
    { icon: Target, title: "Diagnóstico Inicial", description: "Análise completa da sua situação financeira atual, objetivos e perfil de investidor." },
    { icon: TrendingUp, title: "Estratégia Personalizada", description: "Desenvolvimento de um plano financeiro sob medida para seus objetivos específicos." },
    { icon: CheckCircle, title: "Acompanhamento Contínuo", description: "Monitoramento regular dos resultados e ajustes estratégicos quando necessário." },
  ],
  contact: {
    whatsappLink: "https://wa.me/5522992180488?text=Ol%C3%A1%2C%20Luciano!%20Gostaria%20de%20agendar%20uma%20conversa%20inicial%20sobre%20consultoria%20financeira.",
  },
};
...
const FeatureItem = ({ children, index }: { children: ReactNode; index: number }) => (
  <MotionWrapper delay={index * 0.1}>
    <li className="flex items-start gap-3">
      <CheckCircle className="h-5 w-5 flex-shrink-0 text-cyan-400 mt-1" />
      <span className="text-gray-300">{children}</span>
    </li>
  </MotionWrapper>
);

const ProcessStepCard = ({ icon: Icon, title, description, index }: { icon: React.ElementType; title: string; description: string; index: number; }) => (
  <MotionWrapper delay={index * 0.15}>
    <Card className="group relative h-full overflow-hidden border border-white/10 bg-white/[.03] text-center transition-all duration-300 hover:border-cyan-400/50 hover:bg-white/5 hover:-translate-y-2">
      <CardContent className="p-8">
        <div className="mb-5 flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-white/10 bg-gray-900 transition-colors duration-300 group-hover:border-cyan-400 group-hover:bg-cyan-900/50">
    ...
    <div className="min-h-screen bg-gray-950 text-white selection:bg-cyan-500/30">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-gray-950/50 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between p-4 px-6 lg:px-8">
          ...
          <Button asChild className="shadow-sm shadow-cyan-500/20 transition-all hover:shadow-md hover:shadow-cyan-500/40">
            <Link href="#contact">
      ...
      <main className="relative z-10 pt-20 sm:pt-24">
        <section className="mx-auto max-w-7xl px-6 py-16 text-center lg:px-8 lg:py-24">
          <MotionWrapper>
            <Target className="mx-auto h-14 w-14 text-cyan-400" />
            ...
            <div className="lg:col-span-3">
              <MotionWrapper>
                ...
                <div className="prose prose-invert mt-6 max-w-none text-gray-300 text-lg space-y-4">
                  <p>Com mais de <span className="font-semibold text-white">duas décadas de experiência</span>, ofereço um serviço personalizado que vai muito além de simples conselhos.</p>
                  <p>Cada consultoria é <span className="font-semibold text-cyan-400">única e adaptada à sua realidade</span>, considerando seus objetivos, perfil de risco e sonhos futuros.</p>
                  <p>Meu compromisso é exclusivamente com <span className="font-semibold text-white">seu sucesso financeiro</span> e sua independência para tomar as melhores decisões.</p>
                </div>
              </MotionWrapper>
              ...
              <MotionWrapper delay={0.2}>
                <div className="mt-10 rounded-2xl border border-white/10 bg-white/[.03] p-8">
                  ...
                  <ul className="mt-6 space-y-4">
                    {pageData.services.map((service, i) => <FeatureItem key={i} index={i}>{service}</FeatureItem>)}
                  </ul>
                </div>
              </MotionWrapper>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-6 py-24 sm:py-32 lg:px-8">
          ...
          <div className="relative mt-16 grid grid-cols-1 gap-8 md:grid-cols-3">
            {pageData.processSteps.map((item, i) => <ProcessStepCard key={item.title} {...item} index={i} />)}
          </div>
        </section>
              ...
              <div className="rounded-[23px] bg-gray-950/90 px-6 py-24 sm:px-16">
                 <MotionWrapper>
                   <div className="mx-auto max-w-2xl">
                     ...
                     <h2 className="mt-6 text-3xl font-bold tracking-tight text-white sm:text-4xl">Pronto para transformar sua vida financeira?</h2>
                     <p className="mt-6 text-lg leading-8 text-gray-300">
                       Meus clientes alcançam, em média, <span className="font-semibold text-white">rentabilidade superior a 15% ao ano</span>. Agende uma conversa inicial gratuita e descubra como posso te ajudar.
                     </p>
