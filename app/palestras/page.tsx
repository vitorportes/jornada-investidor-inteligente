...
const pageData = {
  ...
  contact: {
    ...
    subtitle:
      "Entre em contato e vamos conversar sobre como posso contribuir para o crescimento financeiro da sua equipe ou comunidade.",
    whatsappLink:
      "https://wa.me/5522992180488?text=Ol%C3%A1%2C%20Luciano!%20Gostaria%20de%20contratar%20uma%20palestra%20sobre%20educa%C3%A7%C3%A3o%20financeira.",
  },
}
...
const TopicItem = ({ children, index }: { children: ReactNode; index: number }) => (
  <MotionWrapper delay={index * 0.05}>
    <div className="flex items-center gap-3">
      <CheckCircle className="h-5 w-5 flex-shrink-0 text-[#e3b061]" />
...
export default function PalestrasPageV1_1() {
  return (
    <div className="min-h-screen bg-gray-950 text-white selection:bg-[#e3b061]/30">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-[#e3b061]/50 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between p-4 px-6 lg:px-8">
          ...
          <Button
            asChild
            className="hidden sm:inline-flex shadow-sm shadow-[#e3b061]/20 transition-all hover:shadow-md hover:shadow-[#e3b061]/40"
          >
            <Link href="#contact">
      ...
      <main className="relative z-10 pt-24 sm:pt-28">
        ...
        <section className="mx-auto max-w-7xl px-6 py-16 text-center lg:px-8 lg:py-20">
          <MotionWrapper>
            <Mic className="mx-auto h-14 w-14 text-[#e3b061]" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                  ...
                  {pageData.about.paragraphs.map((p, i) => (
                    <p
                      ...
                      dangerouslySetInnerHTML={{
                        __html: p
                          .replace(
                            /((?:duas décadas|experiências que mudam mentalidades|casos reais e estratégias práticas|20 anos de experiência))/g,
                            '<span class="font-semibold text-white">$1</span>',
                          )
                          .replace(
                            /(independência financeira)/g,
                            '<span class="font-semibold text-[#e3b061]">$1</span>',
                          ),
                      }}
                    />
                  ))}
                ...
                <div className="grid grid-cols-2 gap-6 pt-4">
                  {pageData.stats.map((stat, i) => (
                    <div key={i} className="text-center rounded-lg bg-white/[.03] p-4 border border-white/10">
                      <stat.icon className="mx-auto h-8 w-8 text-[#e3b061] mb-2" />
            ...
            <MotionWrapper delay={0.1}>
              <div className="relative aspect-[4/5] w-full lg:sticky lg:top-28">
                <Image
                  src={pageData.about.image.src || "/placeholder.svg"}
                  alt={pageData.about.image.alt}
                  fill
                  className="rounded-2xl object-cover shadow-2xl shadow-[#e3b061]/40"
